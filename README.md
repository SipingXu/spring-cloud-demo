...
#### Notes
- By default, all microservices share the same database. And you can cusomize the deployment that each microservice uses its own database.
- In this project, I use ArangoDB as a primary database for each service. It might also make sense to have a polyglot persistence architecture (сhoose the type of db that is best suited to service requirements).
...
Each client in the demo (account-service, statistics-service, notification-service and browser) has a scope: `server` for backend services, and `ui` - for the browser. So we can also protect controllers from external access, for example:
